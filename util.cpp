...
#else
    const char* pszModule = "ThousandBitcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ThousandBitcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ThousandBitcoin
    // Mac: ~/Library/Application Support/ThousandBitcoin
    // Unix: ~/.ThousandBitcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ThousandBitcoinData";
#else
    fs::path pathRet;
    ...
    return pathRet / "ThousandBitcoinData";
#else
    // Unix
    return pathRet / ".ThousandBitcoinData";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ThousandBitcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ThousandBitcoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong ThousandBitcoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("ThousandBitcoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
