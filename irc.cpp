...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("ThousandBitcoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ThousandBitcoinTEST2\r");
            Send(hSocket, "WHO #ThousandBitcoinTEST2\r");
        } else {
            // randomly join #ThousandBitcoin00-#ThousandBitcoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #ThousandBitcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ThousandBitcoin%02d\r", channel_number).c_str());
        }
